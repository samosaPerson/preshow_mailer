...
Preview a sample email (rendered): https://samosaperson.github.io/preshow_mailer/index.html

## Project Value Proposition

The **Pre-Show Mailer** solves the common problem of audience members missing key logistical details (parking, weather, start time). By integrating real-time API data (Weather, Nearby Dining) and automating the generation of nicely formatted, accessible HTML email content, the tool ensures every patron arrives informed and ready for the show.
...
1) **Dash Dashboard (recommended for staff)**
   ...
   - Use **Copy HTML** if you plan to paste directly into a webmail editor (Gmail/Outlook "Edit as HTML").
...
### Paste Into Webmail ("Edit as HTML" via Inspect Element)

If your email client doesn't accept file uploads and you need to paste HTML directly:

1) In the dashboard, click **Copy HTML**.
2) Open your email draft in the browser (Gmail/Outlook web).
3) Right-click inside the message body → **Inspect**.
4) In DevTools, select the message body container, then right-click it → **Edit as HTML**.
5) Paste the copied HTML and close DevTools.

Notes:
- The copied HTML is a fragment (no `<html>`, `<head>`, or `<body>`), which is required for "Edit as HTML".
- If the layout looks narrow, increase the compose window width before pasting.
...
For the macOS one-file build and GitHub release checklist, see `RELEASE.md`.

### If macOS blocks the downloaded app
After you download/unzip the release from GitHub, macOS may quarantine the `.app` because it is unsigned. To run it:
```bash
xattr -dr com.apple.quarantine /path/to/preshow-mailer.app
open /path/to/preshow-mailer.app
```
If Finder still warns, Control-click the app → Open → Open. For user-friendly distribution without these steps, sign and notarize the app with an Apple Developer ID before publishing.
